<?php
  ...
  */
	...
	// TinyModel_diff() -- perform the diff
	// ------------------------------------------------

	function TinyModel_diff() {
		if (!TinyModel::$pdo) {
			Helpers::p(Helpers::clr('Error:', 'red'),  " TinyModel::\$pdo is null\n");
			return;
		}
		...
		// ECHO wrapper -- sends all args to echo unless --silent mode
		$ECHO = function() {
		...
		$f_print_diff_and_get_warnings = function($diff) {
			$warnings = [
				...
				foreach ($diff->table_alterations as $cla => $details) {
					...
					if (isset($details['modify_cols']) && count($details['modify_cols']) > 0) {
						...
						foreach ($details['modify_cols'] as $colname => $mdfcn) {
							Helpers::p("         ",
							           str_repeat(' ', $pad_length - mb_strlen($colname)),
		...
		$warnings = $f_print_diff_and_get_warnings($diff);
		if ($dangerous_modifs_present = (max($warnings) > 0)) {
			...
			if ($should_prompt) {
				...
				$ECHO(Helpers::clr('Really sure? ', 'normal'));
				$response = readline();
				if (substr($response, 0, 1) != 'y') {
					$ECHO("OK, bye.\n\n");
					exit;
		...
		$f_apply_diff = function($diff, $pdo) {
			$statements = [ ];
			$errors = [ ];
			...
			foreach ($statements as $q) {
				...
				if (!$r) {
					...
					if ($should_prompt) {
						$ECHO(Helpers::clr("Error: ", 'red'), "Couldn't execute the following SQL statement:\n");
						$ECHO('  ', $q, "\n");
		...
		$ECHO("\n");
		$sql_errors = $f_apply_diff($diff, Tinymodel::$pdo);

		if (count($sql_errors) == 0) {
			$ECHO(Helpers::clr('Success. ', 'green'),
